...
async function biometricLogin() {
  ...
  try {
    ...
    // Force prepare explicit array objects
    const standardAllowCredentials = [];
    if (options.allowCredentials && options.allowCredentials.length > 0) {
      for (const cred of options.allowCredentials) {
        let cleanIdString = (typeof cred.id === 'string') ? cred.id : cred.id?.id;
        if (!cleanIdString) continue;

        standardAllowCredentials.push({
          type: "public-key",
          id: base64urlToUint8Array(cleanIdString), // Pure buffer assignment
    ...
    const publicKey = {
      ...
      allowCredentials: standardAllowCredentials
    };

    console.log("Verified object types:", publicKey.allowCredentials[0].id instanceof Uint8Array);
